...
public class AIPlayerController : MonoBehaviour
{
    ...
    public PlayerInput playerInput;     // Optional: will be disabled while AI is enabled
    ...
    private void UpdateMovement()
    {
        ...
        {
            float tooFar = preferredDistance + distanceDeadZone;
            float tooClose = preferredDistance - distanceDeadZone;
    ...
    private IEnumerator DecideAction()
    {
        ...
        {
            // These bools exist on both controllers in your scene.
            opponentAttacking = opponent.animator.GetBool("Attack");
            opponentSpecial = opponent.animator.GetBool("special");
        }

        // Sometimes step back even if we are in good range (adds variety).
        bool nearOpponent = distance <= attackRange * 1.15f;
        if (nearOpponent && Time.time >= _nextRetreatAllowedTime && Random.value < retreatChance)
        {
            ...
            // Let movement system do the retreat; just wait a bit to avoid spamming actions.
            yield return new WaitForSeconds(0.1f);
            _isActing = false;
            yield break;
        }

        // Defensive reaction when close (simple heuristic).
        float blockChance = blockReactionChance;
        if (!opponentAttacking && !opponentSpecial)
        {
            // Don't "randomly" block too often when enemy isn't doing anything.
            blockChance *= 0.25f;
        }
        else
        {
            // If opponent is attacking, block a bit more reliably.
        ...
        if (distance <= attackRange)
        {
            ...
            if (canUseSpecial && roll < specialChance)
            {
            ...
            // After an attack, sometimes step back briefly (better spacing / less corner-sticking).
